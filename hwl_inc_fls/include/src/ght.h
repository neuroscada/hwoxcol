...
#pragma message("Entering Header file: ght.h")
#endif
...
#ifdef GHT_STARTFLAG
#pragma message("ERROR: Partially processed: ght.h")
#else
#define GHT_STARTFLAG

/*
@(#)ght.h	main.SVR_MASTERPROJ_Integration.2
BEGIN_DOC
---------------------------------------------------------------------------
...
/* Define constants */
#define GHT_MAX_HOSTS       100
#define GHT_UDP             0
#define GHT_TCP             1
#define GHT_MAX_HOST_LINKS  4
#define GHT_HOST_NAME_LEN	NADS_HOST_NAME_LEN
#define GHT_HOST_BASENAME_LEN	(GHT_HOST_NAME_LEN-2)
...
struct net_host
{
    ...
    u_short portWrite;  /* port number for write requests, NETAPI only */

    int nads_version[GHT_MAX_HOST_LINKS];
                        /* NADS version in use
                            0 = NADS_VERS_UNKNOWN   - Not yet discovered (default)
                            1 = NADS_VERS1          - version 1 (16 bit point numbers)
                            2 = NADS_VERS2          - version 2 (32 bit point numbers) */

    CRITICAL_SECTION cs;    /* critical section to serialise access to this record */
...
/* Functions to support write port functionality for NETAPI */
EXTERN_C int GHTGetCurrentPort_Write _DECLARE((int nIndex,u_short *pPortWrite));
EXTERN_C int GHTSetCurrentPort_Write _DECLARE((int nIndex,u_short pPortWrite));

/* Functions to support NADS version tracking */
EXTERN_C int GHTReadCurrentLnkNADSVersion _DECLARE((int iIndex, int *pnNADSv));
EXTERN_C int GHTWriteCurrentLnkNADSVersion _DECLARE((int iIndex, int iNADSv));
